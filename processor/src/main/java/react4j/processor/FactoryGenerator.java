...
final class FactoryGenerator {
    ...
    private static void buildConstructor(@Nonnull final ProcessingEnvironment processingEnv, @Nonnull final ViewDescriptor descriptor, @Nonnull final TypeSpec.Builder builder, @Nonnull final ExecutableElement constructor) {
    ...
    private static void buildFields(@Nonnull final ProcessingEnvironment processingEnv, @Nonnull final TypeSpec.Builder builder, @Nonnull final ViewDescriptor descriptor, @Nonnull final ExecutableElement constructor) {
        for (final var parameter : descriptor.getInjectableConstructorParameters()) {
            final var field = FieldSpec.builder(TypeName.get(parameter.asType()), parameter.getSimpleName().toString(), Modifier.PRIVATE, Modifier.FINAL);
    ...
    private static void buildCreateMethod(@Nonnull final ViewDescriptor descriptor, @Nonnull final TypeSpec.Builder builder) {
        builder.addMethod(MethodSpec.methodBuilder("create").addModifiers(Modifier.STATIC).addAnnotation(GeneratorUtil.NONNULL_CLASSNAME).addParameter(ParameterSpec.builder(NATIVE_VIEW_CLASSNAME, "view", Modifier.FINAL).addAnnotation(GeneratorUtil.NONNULL_CLASSNAME).build()).returns(descriptor.getEnhancedClassName()).addStatement("return InjectSupport.create( view )").build());
    }
    ...
    private static TypeSpec buildInjectSupport(@Nonnull final ViewDescriptor descriptor) {
        ...
        final var builder = TypeSpec.classBuilder("InjectSupport").addModifiers(Modifier.PRIVATE, Modifier.STATIC, Modifier.FINAL);
    ...
    private static MethodSpec.Builder buildSetFactoryMethod(@Nonnull final ViewDescriptor descriptor) {
        final var method = MethodSpec.methodBuilder("setFactory").addModifiers(Modifier.PRIVATE, Modifier.STATIC).addParameter(ParameterSpec.builder(descriptor.getFactoryClassName(), "factory", Modifier.FINAL).addAnnotation(GeneratorUtil.NONNULL_CLASSNAME).build());
        final var block = CodeBlock.builder();
        block.beginControlFlow("if ( $T.shouldCheckInvariants() )", REACT_CLASSNAME);
        block.addStatement("$T.invariant( () -> null == c_factory, () -> \"Attempted to instantiate the React4j " + "view factory for the view named '$N' a second time\" )", GUARDS_CLASSNAME, descriptor.getName());
    ...
    private static MethodSpec.Builder buildInjectCreateMethod(@Nonnull final ViewDescriptor descriptor) {
        ...
        for (final var input : descriptor.getConstructorInputs()) {
            ...
            if (!input.getType().getKind().isPrimitive() && !input.isNonNull()) {
                ...
                extraction.addStatement("$N = null != view.inputs().getAsAny( $T.Inputs.$N ) ? " + "view.inputs().getAsAny( $T.Inputs.$N ).$N() : null", localName, descriptor.getEnhancedClassName(), input.getConstantName(), descriptor.getEnhancedClassName(), input.getConstantName(), convertMethodName);
                extraction.nextControlFlow("else");
                extraction.addStatement("$N = $T.uncheckedCast( view.inputs().getAsAny( $T.Inputs.$N ) )", localName, JS_CLASSNAME, descriptor.getEnhancedClassName(), input.getConstantName());
                extraction.endControlFlow();
                method.addCode(extraction.build());
            } else {
                method.addStatement("final $T $N = view.inputs().getAsAny( $T.Inputs.$N ).$N()", TypeName.get(input.getType()), localName, descriptor.getEnhancedClassName(), input.getConstantName(), convertMethodName);
            }
        }
        ...
        for (final VariableElement parameter : constructor.getParameters()) {
            args.append(", ");
            final var input = descriptor.getConstructorInputs().stream().filter(d -> parameter.equals(d.getParameter())).findAny().orElse(null);
            if (null != input) {
                args.append("$N");
    ...
    private static String getConverter(@Nonnull final InputDescriptor input) {
        return switch(input.getType().getKind()) {
            ...
            case TYPEVAR, ARRAY ->
                "cast";
            case DECLARED ->
                input.getType().toString().equals("java.lang.String") ? "asString" : "cast";
            default ->
                throw new ProcessorException("Return type of @Input method is not yet handled. Type: " + input.getType().getKind(), input.getElement());
        };
    }
}
